...
import com.isix.easyGym.common.RandomImageService;
...
public class DetailControllerImpl implements DetailController {
	...
	private static String ARTICLE_IMG_REPO = "src" + File.separator + "main" + File.separator + "resources" + File.separator + "static" + File.separator + "images" + File.separator + "detail";

	@Autowired private DetailDTO detailDTO;
	@Autowired private DetailServiceImpl detailService;
	@Autowired private DetailDAO detailDAO;
	@Autowired private DetailDibsDTO detailDibsDTO;
	@Autowired private MemberDTO memberDTO;
	@Autowired private MemberServiceImpl memberService;
	@Autowired private MemberDAO memberDAO;
	@Autowired private PayformDTO payformDTO;
	@Autowired private PayformServiceImpl payformService;
	@Autowired private DetailReviewDTO detailReviewDTO;
	@Autowired private DetailImageDTO detailImageDTO;
	@Autowired private MemberOperDTO memberOperDTO;
	@Autowired private RandomImageService randomImageService;
	...
	public ModelAndView detailForm(
		...
		DetailDTO details = detailService.viewDetail(detailNo);
		...
		// 1. 서비스로부터 전체 랜덤 이미지 경로 리스트를 가져옵니다.
		List<String> allRandomPaths = randomImageService.getAllCachedImagePaths();
		// 2. 이 리스트를 JSON 문자열로 변환합니다.
		String allRandomPathsJson = objectMapper.writeValueAsString(allRandomPaths);
		// 3. JSON 문자열을 모델에 추가합니다.
		mav.addObject("allRandomImagePathsJson", allRandomPathsJson);
	...
	public ModelAndView searchData(@RequestParam("query") String query,
		...
		if (selectedThing != null && !selectedThing.isEmpty()) {
			for (DetailDTO dto : selectedThing) {
				dto.setRandomImagePath(randomImageService.getRandomImagePath());
			}
		}
